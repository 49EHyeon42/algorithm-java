...
public class Main {
    ...
    private static void bfs() {
        queue.offer(new Vertex(0, 0, graph[0][0] - '0'));
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < 2; i++) {
                ...
                if (graph[nextY][nextX] < 48) { // ASCII, 다음이 연산자라 라면
                    queue.offer(new Vertex(nextY, nextX, currentVertex.value, graph[nextY][nextX]));
                } else {
                    queue.offer(new Vertex(nextY, nextX, calculation(currentVertex.operator, currentVertex.value, graph[nextY][nextX] - '0')));
                }
            }
        }
    }
