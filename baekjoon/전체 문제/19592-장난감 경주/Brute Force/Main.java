...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (T-- > 0) {
            ...
            int minBoost = Integer.MAX_VALUE;

            for (int boost = mySpeed + 1; boost <= Y; boost++) {
                if (getSecond(X - boost, mySpeed) + 1 < minSecond) {
                    minBoost = boost;
                    break;
                }
            }

            sb.append(minBoost == Integer.MAX_VALUE ? -1 : minBoost).append('\n');
        }
